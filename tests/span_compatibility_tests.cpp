...
// Below are tests that verify the gsl interface support the same things as the
// std Ranges and Concepts support need to be added later.
...
// int*(*) [], int const* const(*)[] was identified as an issue in CWG330 and
// the resolution was provided with N4261.
...
void ArrayConvertibilityCheck() {
  ...
  if constexpr (std::is_convertible<T*(*)[], T const* const(*)[]>::value) {
    ...
    CHECK(sp_const_nullptr_1.data() == stl_nullptr.data());
    CHECK(sp_const_nullptr_1.size() == 3);

    span<const T* const> sp_const_nullptr_2{std::as_const(stl_nullptr)};
    CHECK(sp_const_nullptr_2.data() == stl_nullptr.data());
    CHECK(sp_const_nullptr_2.size() == 3);

    static_assert(
        std::is_same<decltype(span{stl_nullptr}), span<T*, 3>>::value,
        "std::is_same< decltype(span{stl_nullptr}), span<T*, 3>>::value");
    static_assert(std::is_same<decltype(span{std::as_const(stl_nullptr)}),
                               span<T* const, 3>>::value,
                  "std::is_same< decltype(span{std::as_const(stl_nullptr)}), "
                  "span<T* const, "
                  "3>>::value");
  }
#endif
...
TEST_CASE("span_compatibility_tests, assertion_tests") {
  int arr[3]{10, 20, 30};
  ...
  {
    gsl::span<int> sp_dyn;
    CHECK(sp_dyn.data() == nullptr);
    CHECK(sp_dyn.size() == 0);
    CHECK(sp_dyn.empty());
  }
  {
    gsl::span<int, 0> sp_zero;
    CHECK(sp_zero.data() == nullptr);
    CHECK(sp_zero.size() == 0);
    CHECK(sp_zero.empty());

    gsl::span<int> sp_dyn_a(arr, 3);
    gsl::span<int> sp_dyn_b(begin(arr), 3);
    CHECK(sp_dyn_a.data() == std::begin(arr));
    CHECK(sp_dyn_b.data() == std::begin(arr));
    CHECK(sp_dyn_a.size() == 3);
    CHECK(sp_dyn_b.size() == 3);

    gsl::span<int, 3> sp_three_a(arr, 3);
    gsl::span<int, 3> sp_three_b(begin(arr), 3);
    CHECK(sp_three_a.data() == std::begin(arr));
    CHECK(sp_three_b.data() == std::begin(arr));
    CHECK(sp_three_a.size() == 3);
    CHECK(sp_three_b.size() == 3);

    gsl::span<const int> sp_const_a(arr, 3);
    gsl::span<const int> sp_const_b(begin(arr), 3);
    CHECK(sp_const_a.data() == std::begin(arr));
    CHECK(sp_const_b.data() == std::begin(arr));
    CHECK(sp_const_a.size() == 3);
    CHECK(sp_const_b.size() == 3);

#if __cplusplus >= 201703l
    gsl::span<const int> sp_const_c(std::as_const(arr), 3);
    CHECK(sp_const_c.data() == std::begin(arr));
    CHECK(sp_const_c.size() == 3);
#endif  // __cplusplus >= 201703l

    gsl::span<const int> sp_const_d(cbegin(arr), 3);
    CHECK(sp_const_d.data() == std::begin(arr));
    CHECK(sp_const_d.size() == 3);
  }
  {
  ...
  }
  {
    ...
    CHECK(sp_three_a.data() == std::begin(arr));
    CHECK(sp_three_b.data() == stl.data());
    CHECK(sp_three_a.size() == 3);
    CHECK(sp_three_b.size() == 3);

    gsl::span<const int> sp_const_w(arr);
    gsl::span<const int> sp_const_y(stl);
    CHECK(sp_const_w.data() == std::begin(arr));
    CHECK(sp_const_y.data() == stl.data());
    CHECK(sp_const_w.size() == 3);
    CHECK(sp_const_y.size() == 3);

#if __cplusplus >= 201703l
    gsl::span<const int> sp_const_x(std::as_const(arr));
    CHECK(sp_const_x.data() == std::begin(arr));
    CHECK(sp_const_x.size() == 3);

    gsl::span<const int> sp_const_z(std::as_const(stl));
    CHECK(sp_const_z.data() == stl.data());
    CHECK(sp_const_z.size() == 3);
#endif  // __cplusplus >= 201703l
  }
  {
    ...
    CHECK(sp_a.data() == std::begin(arr));
    CHECK(sp_b.data() == std::begin(arr));
    CHECK(sp_c.data() == std::begin(arr));
    CHECK(sp_d.data() == std::begin(arr));
    CHECK(sp_e.data() == std::begin(arr));
    CHECK(sp_f.data() == std::begin(arr));
    CHECK(sp_g.data() == std::begin(arr));
    CHECK(sp_h.data() == std::begin(arr));
    CHECK(sp_i.data() == std::begin(arr));
    CHECK(sp_a.size() == 3);
    CHECK(sp_b.size() == 3);
    CHECK(sp_c.size() == 3);
    CHECK(sp_d.size() == 3);
  ...
  }
  {
    ...
    CHECK(sp_dyn.data() == std::begin(arr));
    CHECK(sp_three.data() == std::begin(arr));
    CHECK(sp_const_dyn.data() == std::begin(arr));
    CHECK(sp_const_three.data() == std::begin(arr));
    CHECK(sp_dyn.size() == 3);
    CHECK(sp_three.size() == 3);
    CHECK(sp_const_dyn.size() == 3);
    CHECK(sp_const_three.size() == 3);
    ...
    CHECK(sp_dyn.data() == std::begin(other));
    CHECK(sp_three.data() == stl.data());
    CHECK(sp_const_dyn.data() == std::begin(other));
    CHECK(sp_const_three.data() == stl.data());
    CHECK(sp_dyn.size() == 4);
    CHECK(sp_three.size() == 3);
    CHECK(sp_const_dyn.size() == 4);
    CHECK(sp_const_three.size() == 3);
  }
  {
    ...
    CHECK(*it_three == stl[0]);
    CHECK(it_three[2] == stl[2]);
  }

  {
    ...
    static_assert(noexcept(sp_dyn.first<3>()),
                  "noexcept(sp_dyn.first<3>())");  // strengthened
    static_assert(noexcept(sp_nine.first<4>()),
                  "noexcept(sp_nine.first<4>())");  // strengthened
    static_assert(noexcept(sp_dyn.first(5)),
                  "noexcept(sp_dyn.first(5))");  // strengthened
    static_assert(noexcept(sp_nine.first(6)),
                  "noexcept(sp_nine.first(6))");  // strengthened
    ...
    CHECK(first_3.data() == std::begin(sequence));
    CHECK(first_4.data() == std::begin(sequence));
    CHECK(first_5.data() == std::begin(sequence));
    CHECK(first_6.data() == std::begin(sequence));
    CHECK(first_3.size() == 3);
    CHECK(first_4.size() == 4);
    CHECK(first_5.size() == 5);
    CHECK(first_6.size() == 6);

    auto last_3 = sp_dyn.last<3>();
    auto last_4 = sp_nine.last<4>();
    auto last_5 = sp_dyn.last(5);
    auto last_6 = sp_nine.last(6);
    static_assert(noexcept(sp_dyn.last<3>()),
                  "noexcept(sp_dyn.last<3>())");  // strengthened
    static_assert(noexcept(sp_nine.last<4>()),
                  "noexcept(sp_nine.last<4>())");  // strengthened
    static_assert(noexcept(sp_dyn.last(5)),
                  "noexcept(sp_dyn.last(5))");  // strengthened
    static_assert(noexcept(sp_nine.last(6)),
                  "noexcept(sp_nine.last(6))");  // strengthened
    ...
    CHECK(last_3.data() == std::begin(sequence) + 6);
    CHECK(last_4.data() == std::begin(sequence) + 5);
    CHECK(last_5.data() == std::begin(sequence) + 4);
    CHECK(last_6.data() == std::begin(sequence) + 3);
    CHECK(last_3.size() == 3);
    CHECK(last_4.size() == 4);
    CHECK(last_5.size() == 5);
    CHECK(last_6.size() == 6);
    ...
    static_assert(noexcept(sp_dyn.subspan(5)),
                  "noexcept(sp_dyn.subspan(5))");  // strengthened
    static_assert(noexcept(sp_nine.subspan(6)),
                  "noexcept(sp_nine.subspan(6))");  // strengthened
    ...
    CHECK(offset_3.data() == std::begin(sequence) + 3);
    CHECK(offset_4.data() == std::begin(sequence) + 4);
    CHECK(offset_5.data() == std::begin(sequence) + 5);
    CHECK(offset_6.data() == std::begin(sequence) + 6);
    CHECK(offset_3.size() == 6);
    CHECK(offset_4.size() == 5);
    CHECK(offset_5.size() == 4);
    CHECK(offset_6.size() == 3);
    ...
    CHECK(subspan_3.data() == std::begin(sequence) + 3);
    CHECK(subspan_4.data() == std::begin(sequence) + 4);
    CHECK(subspan_5.data() == std::begin(sequence) + 5);
    CHECK(subspan_6.data() == std::begin(sequence) + 6);
    CHECK(subspan_3.size() == 2);
    CHECK(subspan_4.size() == 2);
    CHECK(subspan_5.size() == 2);
    CHECK(subspan_6.size() == 2);

    static_assert(noexcept(sp_dyn.size()), "noexcept(sp_dyn.size())");
    static_assert(noexcept(sp_dyn.size_bytes()),
                  "noexcept(sp_dyn.size_bytes())");
    static_assert(noexcept(sp_dyn.empty()), "noexcept(sp_dyn.empty())");
    static_assert(noexcept(sp_dyn[0]), "noexcept(sp_dyn[0])");  // strengthened
    static_assert(noexcept(sp_dyn.front()),
                  "noexcept(sp_dyn.front())");  // strengthened
    static_assert(noexcept(sp_dyn.back()),
                  "noexcept(sp_dyn.back())");  // strengthened
    ...
    static_assert(noexcept(sp_nine.size_bytes()),
                  "noexcept(sp_nine.size_bytes())");
    static_assert(noexcept(sp_nine.empty()), "noexcept(sp_nine.empty())");
    static_assert(noexcept(sp_nine[0]),
                  "noexcept(sp_nine[0])");  // strengthened
    static_assert(noexcept(sp_nine.front()),
                  "noexcept(sp_nine.front())");  // strengthened
    static_assert(noexcept(sp_nine.back()),
                  "noexcept(sp_nine.back())");  // strengthened
    ...
    CHECK(sp_dyn.size() == 9);
    CHECK(sp_nine.size() == 9);

    CHECK(sp_dyn.size_bytes() == 9 * sizeof(int));
    CHECK(sp_nine.size_bytes() == 9 * sizeof(int));
    ...
    CHECK(sp_dyn.front() == 10);
    CHECK(sp_nine.front() == 10);

    CHECK(sp_dyn.back() == 90);
    CHECK(sp_nine.back() == 90);

    CHECK(&sp_dyn.front() == std::begin(sequence));
    CHECK(&sp_nine.front() == std::begin(sequence));
    CHECK(&sp_dyn[4] == std::begin(sequence) + 4);
    CHECK(&sp_nine[4] == std::begin(sequence) + 4);
    CHECK(&sp_dyn.back() == std::begin(sequence) + 8);
    CHECK(&sp_nine.back() == std::begin(sequence) + 8);

    CHECK(sp_dyn.data() == std::begin(sequence));
    CHECK(sp_nine.data() == std::begin(sequence));

    CHECK(*sp_dyn.begin() == 10);
    CHECK(*sp_nine.begin() == 10);

    CHECK(sp_dyn.end()[-2] == 80);
    CHECK(sp_nine.end()[-2] == 80);

    CHECK(*sp_dyn.rbegin() == 90);
    CHECK(*sp_nine.rbegin() == 90);

    CHECK(sp_dyn.rend()[-2] == 20);
    CHECK(sp_nine.rend()[-2] == 20);
    ...
    static_assert(
        ...
        "is_same<decltype(sp_nine.begin()), gsl::span<int, "
        "9>::iterator>::value");
    static_assert(
    ...
    static_assert(
        ...
        "is_same<decltype(sp_nine.end()), gsl::span<int, 9>::iterator>::value");
    static_assert(is_same<decltype(sp_dyn.rbegin()),
                          gsl::span<int>::reverse_iterator>::value,
                  "is_same<decltype(sp_dyn.rbegin()), "
                  "gsl::span<int>::reverse_iterator>::value");
    static_assert(is_same<decltype(sp_nine.rbegin()),
                          gsl::span<int, 9>::reverse_iterator>::value,
                  "is_same<decltype(sp_nine.rbegin()), gsl::span<int, "
                  "9>::reverse_iterator>::value");
    static_assert(is_same<decltype(sp_dyn.rend()),
                          gsl::span<int>::reverse_iterator>::value,
                  "is_same<decltype(sp_dyn.rend()), "
                  "gsl::span<int>::reverse_iterator>::value");
    static_assert(is_same<decltype(sp_nine.rend()),
                          gsl::span<int, 9>::reverse_iterator>::value,
                  "is_same<decltype(sp_nine.rend()), gsl::span<int, "
                  "9>::reverse_iterator>::value");
  }
  {
    int sequence[9]{10, 20, 30, 40, 50, 60, 70, 80, 90};
    ...
    static_assert(noexcept(as_bytes(sp_const_nine)),
                  "noexcept(as_bytes(sp_const_nine))");
    static_assert(noexcept(as_writable_bytes(sp_dyn)),
                  "noexcept(as_writable_bytes(sp_dyn))");
    static_assert(noexcept(as_writable_bytes(sp_nine)),
                  "noexcept(as_writable_bytes(sp_nine))");
    ...
    CHECK(sp_1.data() == reinterpret_cast<const byte*>(begin(sequence)));
    CHECK(sp_2.data() == reinterpret_cast<const byte*>(begin(sequence)));
    CHECK(sp_3.data() == reinterpret_cast<const byte*>(begin(sequence)));
    CHECK(sp_4.data() == reinterpret_cast<const byte*>(begin(sequence)));
    CHECK(sp_5.data() == reinterpret_cast<byte*>(begin(sequence)));
    CHECK(sp_6.data() == reinterpret_cast<byte*>(begin(sequence)));

    CHECK(sp_1.size() == SizeBytes);
    CHECK(sp_2.size() == SizeBytes);
    CHECK(sp_3.size() == SizeBytes);
    CHECK(sp_4.size() == SizeBytes);
    CHECK(sp_5.size() == SizeBytes);
    CHECK(sp_6.size() == SizeBytes);
  }
}

// assertions for span's definition
static_assert(
    std::is_same<decltype(gsl::dynamic_extent), const std::size_t>::value,
    "gsl::dynamic_extent must be respresented as std::size_t");
static_assert(
    gsl::dynamic_extent == static_cast<std::size_t>(-1),
    "gsl::dynamic_extent must be defined as the max value of std::size_t");

static_assert(
    std::is_same<decltype(gsl::span<int>::extent), const std::size_t>::value,
    "Ensure that the type of  gsl::span::extent is std::size_t");
static_assert(
    gsl::span<int>::extent == gsl::dynamic_extent,
    "gsl::span<int>::extent should be equivalent to gsl::dynamic_extent");

static_assert(
    std::is_same<decltype(gsl::span<int, 3>::extent), const std::size_t>::value,
    "Ensure that the type of gsl::span::extent is std::size_t");
static_assert(gsl::span<int, 3>::extent == 3,
              "Ensure that span<int, 3>::extent is equal to 3");
...
static_assert(
    std::is_same<gsl::span<int, 3>::difference_type, ptrdiff_t>::value,
...
static_assert(
    std::is_same<gsl::span<int, 3>::const_reference, const int&>::value,
    "span<int, 3>::const_reference should be const int&");

static_assert(
    std::is_same<gsl::span<const int>::element_type, const int>::value,
...
static_assert(
    std::is_same<gsl::span<const int>::difference_type, ptrdiff_t>::value,
...
static_assert(
    std::is_same<gsl::span<const int>::const_pointer, const int*>::value,
...
static_assert(
    std::is_same<gsl::span<const int>::const_reference, const int&>::value,
    "span<const int>::const_reference should be const int&");

static_assert(
    std::is_same<gsl::span<const int, 3>::element_type, const int>::value,
...
static_assert(
    std::is_same<gsl::span<const int, 3>::size_type, std::size_t>::value,
    "span<const int, 3>::size_type should be size_t");
static_assert(
    std::is_same<gsl::span<const int, 3>::difference_type, ptrdiff_t>::value,
...
static_assert(
    std::is_same<gsl::span<const int, 3>::const_pointer, const int*>::value,
    "span<const int, 3>::const_pointer should be const int*");
static_assert(
    std::is_same<gsl::span<const int, 3>::reference, const int&>::value,
    "span<const int, 3>::reference should be const int&");
static_assert(
    std::is_same<gsl::span<const int, 3>::const_reference, const int&>::value,
    "span<const int, 3>::const_reference should be const int&");

// assertions for span_iterator
static_assert(
    std::is_same<std::iterator_traits<gsl::span<int>::iterator>::pointer,
                 int*>::value,
...
static_assert(
    ...
    "span<int>::reverse_iterator should equal "
    "std::reverse_iterator<span<int>::iterator>");

static_assert(
    std::is_same<std::iterator_traits<gsl::span<int, 3>::iterator>::pointer,
                 int*>::value,
...
static_assert(
    ...
    "span<int, 3>::reverse_iterator should equal "
    "std::reverse_iterator<span<int, 3>::iterator>");

static_assert(
    std::is_same<std::iterator_traits<gsl::span<const int>::iterator>::pointer,
                 const int*>::value,
    "span<const int>::iterator's pointer should be int*");
static_assert(
    std::is_same<gsl::span<const int>::reverse_iterator,
                 std::reverse_iterator<gsl::span<const int>::iterator>>::value,
    "span<const int>::reverse_iterator should equal "
    "std::reverse_iterator<span<const "
    "int>::iterator>");

static_assert(
    std::is_same<
        std::iterator_traits<gsl::span<const int, 3>::iterator>::pointer,
        const int*>::value,
    "span<const int, 3>::iterator's pointer should be int*");
static_assert(
    std::is_same<
        gsl::span<const int, 3>::reverse_iterator,
        std::reverse_iterator<gsl::span<const int, 3>::iterator>>::value,
    "span<const int, 3>::reverse_iterator should equal "
    "std::reverse_iterator<span<const "
    "int, 3>::iterator>");
...
static_assert(
    std::is_trivially_copyable<gsl::span<const int, 3>::iterator>::value,
    "span<const int, 3>::iterator should be trivially copyable");

// nothrow constructible assertions
static_assert(
    std::is_nothrow_constructible<gsl::span<int>, int*, std::size_t>::value,
    "std::is_nothrow_constructible<gsl::span<int>, int*, std::size_t>");
static_assert(
    std::is_nothrow_constructible<gsl::span<int>, int*, std::uint16_t>::value,
...
static_assert(
    std::is_nothrow_constructible<gsl::span<int>, const gsl::span<int>&>::value,
    "std::is_nothrow_constructible<gsl::span<int>, const gsl::span<int>&>");
static_assert(
    std::is_nothrow_constructible<gsl::span<int>,
                                  const gsl::span<int, 3>&>::value,
    "std::is_nothrow_constructible<gsl::span<int>, const gsl::span<int, 3>&>");
static_assert(std::is_nothrow_constructible<gsl::span<int>,
                                            const gsl::span<int, 500>&>::value,
              "std::is_nothrow_constructible<gsl::span<int>, const "
              "gsl::span<int, 500>&>");
static_assert(
    std::is_nothrow_constructible<gsl::span<int>, std::array<int, 3>&>::value,
    "std::is_nothrow_constructible<gsl::span<int>, std::array<int, 3>&>");

static_assert(
    std::is_nothrow_constructible<gsl::span<int, 3>, int*, std::size_t>::value,
    "std::is_nothrow_constructible<gsl::span<int, 3>, int*, std::size_t>");
static_assert(
    std::is_nothrow_constructible<gsl::span<int, 3>, int*,
                                  std::uint16_t>::value,
    "std::is_nothrow_constructible<gsl::span<int, 3>, int*, std::uint16_t>");
static_assert(
    std::is_nothrow_constructible<gsl::span<int, 3>, int*, int*>::value,
    "std::is_nothrow_constructible<gsl::span<int, 3>, int*, int*>");
static_assert(
    std::is_nothrow_constructible<gsl::span<int, 3>, int (&)[3]>::value,
    "std::is_nothrow_constructible<gsl::span<int, 3>, int(&)[3]>");
static_assert(std::is_nothrow_constructible<gsl::span<int, 3>,
                                            const gsl::span<int, 3>&>::value,
              "std::is_nothrow_constructible<gsl::span<int, 3>, const "
              "gsl::span<int, 3>&>");
static_assert(
    std::is_nothrow_constructible<gsl::span<int, 3>,
                                  std::array<int, 3>&>::value,
    "std::is_nothrow_constructible<gsl::span<int, 3>, std::array<int, 3>&>");

static_assert(
    std::is_nothrow_constructible<gsl::span<const int>, int*,
                                  std::size_t>::value,
    "std::is_nothrow_constructible<gsl::span<const int>, int*, std::size_t>");
static_assert(
    std::is_nothrow_constructible<gsl::span<const int>, int*, int*>::value,
...
static_assert(
    std::is_nothrow_constructible<gsl::span<const int>, int (&)[3]>::value,
    "std::is_nothrow_constructible<gsl::span<const int>, int(&)[3]>");
static_assert(
    std::is_nothrow_constructible<gsl::span<const int>, const int*,
                                  int*>::value,
    "std::is_nothrow_constructible<gsl::span<const int>, const int*, int*>");
static_assert(std::is_nothrow_constructible<gsl::span<const int>, const int*,
                                            const int*>::value,
              "std::is_nothrow_constructible<gsl::span<const int>, const int*, "
              "const int*>");
static_assert(std::is_nothrow_constructible<gsl::span<const int>, const int*,
              ...
              "std::size_t>");
static_assert(
    std::is_nothrow_constructible<gsl::span<const int>,
                                  const int (&)[3]>::value,
    "std::is_nothrow_constructible<gsl::span<const int>, const int(&)[3]>");
static_assert(std::is_nothrow_constructible<gsl::span<const int>,
                                            const gsl::span<int>&>::value,
              "std::is_nothrow_constructible<gsl::span<const int>, const "
              "gsl::span<int>&>");
static_assert(std::is_nothrow_constructible<gsl::span<const int>,
                                            const gsl::span<int, 3>&>::value,
              "std::is_nothrow_constructible<gsl::span<const int>, const "
...
static_assert(std::is_nothrow_constructible<gsl::span<const int>,
              ...
              "gsl::span<int, 500>&>");
static_assert(std::is_nothrow_constructible<gsl::span<const int>,
                                            const gsl::span<const int>&>::value,
              "std::is_nothrow_constructible<gsl::span<const int>, const "
              "gsl::span<const int>&>");
static_assert(std::is_nothrow_constructible<
                  gsl::span<const int>, const gsl::span<const int, 3>&>::value,
              "std::is_nothrow_constructible<gsl::span<const int>, const "
              "gsl::span<const int, 3>&>");
static_assert(
    std::is_nothrow_constructible<gsl::span<const int>,
                                  const gsl::span<const int, 500>&>::value,
    "std::is_nothrow_constructible<gsl::span<const int>, const gsl::span<const "
    "int, 500>&>");
static_assert(
    std::is_nothrow_constructible<gsl::span<const int>,
                                  std::array<int, 3>&>::value,
...
static_assert(std::is_nothrow_constructible<gsl::span<const int, 3>,
                                            const gsl::span<int, 3>&>::value,
              "std::is_nothrow_constructible<gsl::span<const int, 3>, const "
              "gsl::span<int, 3>&>");
static_assert(
    std::is_nothrow_constructible<gsl::span<const int, 3>,
                                  const gsl::span<const int, 3>&>::value,
    "std::is_nothrow_constructible<gsl::span<const int, 3>, const "
    "gsl::span<const int, 3>&>");

static_assert(
    std::is_nothrow_constructible<gsl::span<Base>, Base (&)[3]>::value,
    "std::is_nothrow_constructible<gsl::span<Base>, Base(&)[3]>");
static_assert(
    std::is_nothrow_constructible<gsl::span<Base>,
                                  const gsl::span<Base>&>::value,
    "std::is_nothrow_constructible<gsl::span<Base>, const gsl::span<Base>&>");
static_assert(std::is_nothrow_constructible<gsl::span<Base>,
                                            const gsl::span<Base, 3>&>::value,
              "std::is_nothrow_constructible<gsl::span<Base>, const "
              "gsl::span<Base, 3>&>");
static_assert(
    std::is_nothrow_constructible<gsl::span<Base>, std::array<Base, 3>&>::value,
    "std::is_nothrow_constructible<gsl::span<Base>, std::array<Base, 3>&>");

static_assert(
    std::is_nothrow_constructible<gsl::span<Base, 3>, Base (&)[3]>::value,
    "std::is_nothrow_constructible<gsl::span<Base, 3>, Base(&)[3]>");
static_assert(std::is_nothrow_constructible<gsl::span<Base, 3>,
                                            const gsl::span<Base, 3>&>::value,
              "std::is_nothrow_constructible<gsl::span<Base, 3>, const "
              "gsl::span<Base, 3>&>");
static_assert(
    std::is_nothrow_constructible<gsl::span<Base, 3>,
                                  std::array<Base, 3>&>::value,
    "std::is_nothrow_constructible<gsl::span<Base, 3>, std::array<Base, 3>&>");

static_assert(
    std::is_nothrow_constructible<gsl::span<const Base>, Base (&)[3]>::value,
    "std::is_nothrow_constructible<gsl::span<const Base>, Base(&)[3]>");
static_assert(
    std::is_nothrow_constructible<gsl::span<const Base>,
                                  const Base (&)[3]>::value,
    "std::is_nothrow_constructible<gsl::span<const Base>, const Base(&)[3]>");
static_assert(std::is_nothrow_constructible<gsl::span<const Base>,
                                            const gsl::span<Base>&>::value,
              "std::is_nothrow_constructible<gsl::span<const Base>, const "
              "gsl::span<Base>&>");
static_assert(std::is_nothrow_constructible<gsl::span<const Base>,
                                            const gsl::span<Base, 3>&>::value,
              "std::is_nothrow_constructible<gsl::span<const Base>, const "
              "gsl::span<Base, 3>&>");
static_assert(std::is_nothrow_constructible<
                  gsl::span<const Base>, const gsl::span<const Base>&>::value,
              "std::is_nothrow_constructible<gsl::span<const Base>, const "
              "gsl::span<const Base>&>");
static_assert(
    std::is_nothrow_constructible<gsl::span<const Base>,
                                  const gsl::span<const Base, 3>&>::value,
    "std::is_nothrow_constructible<gsl::span<const Base>, const "
    "gsl::span<const Base, 3>&>");
static_assert(std::is_nothrow_constructible<gsl::span<const Base>,
                                            std::array<Base, 3>&>::value,
              "std::is_nothrow_constructible<gsl::span<const Base>, "
              "std::array<Base, 3>&>");
static_assert(std::is_nothrow_constructible<gsl::span<const Base>,
                                            const std::array<Base, 3>&>::value,
              "std::is_nothrow_constructible<gsl::span<const Base>, const "
              "std::array<Base, 3>&>");

static_assert(std::is_nothrow_constructible<gsl::span<const Base, 3>,
                                            const gsl::span<Base, 3>&>::value,
              "std::is_nothrow_constructible<gsl::span<const Base, 3>, const "
              "gsl::span<Base, 3>&>");
static_assert(
    std::is_nothrow_constructible<gsl::span<const Base, 3>,
                                  const gsl::span<const Base, 3>&>::value,
    "std::is_nothrow_constructible<gsl::span<const Base, 3>, const "
    "gsl::span<const Base, 3>&>");
...
static_assert(
    !std::is_constructible<gsl::span<int>, const int*, const int*>::value,
    "!std::is_constructible<gsl::span<int>, const int*, const int*>");
static_assert(
    !std::is_constructible<gsl::span<int>, const int*, double*>::value,
    "!std::is_constructible<gsl::span<int>, const int*, double*>");
static_assert(
    !std::is_constructible<gsl::span<int>, const int*, std::size_t>::value,
...
static_assert(
    !std::is_constructible<gsl::span<int>, double*, const int*>::value,
...
static_assert(
    !std::is_constructible<gsl::span<int>, double*, std::size_t>::value,
...
static_assert(
    !std::is_constructible<gsl::span<int>, std::size_t, std::size_t>::value,
    "!std::is_constructible<gsl::span<int>, std::size_t, std::size_t>");
static_assert(
    !std::is_constructible<gsl::span<int>, const gsl::span<const int>&>::value,
    "!std::is_constructible<gsl::span<int>, const gsl::span<const int>&>");
static_assert(
    !std::is_constructible<gsl::span<int>,
                           const gsl::span<const int, 3>&>::value,
...
static_assert(
    !std::is_constructible<gsl::span<int>, const gsl::span<double, 3>&>::value,
    "!std::is_constructible<gsl::span<int>, const gsl::span<double, 3>&>");
static_assert(
    !std::is_constructible<gsl::span<int>, std::array<double, 3>&>::value,
    "!std::is_constructible<gsl::span<int>, std::array<double, 3>&>");
static_assert(
    !std::is_constructible<gsl::span<int>, const std::array<int, 3>&>::value,
...
static_assert(
    !std::is_constructible<gsl::span<int, 3>, const int*, int*>::value,
    "!std::is_constructible<gsl::span<int, 3>, const int*, int*>");
static_assert(
    !std::is_constructible<gsl::span<int, 3>, const int*, const int*>::value,
    "!std::is_constructible<gsl::span<int, 3>, const int*, const int*>");
static_assert(
    !std::is_constructible<gsl::span<int, 3>, const int*, std::size_t>::value,
    "!std::is_constructible<gsl::span<int, 3>, const int*, std::size_t>");
static_assert(
    !std::is_constructible<gsl::span<int, 3>, const int*, double*>::value,
    "!std::is_constructible<gsl::span<int, 3>, const int*, double*>");
static_assert(
    !std::is_constructible<gsl::span<int, 3>, const int (&)[3]>::value,
    "!std::is_constructible<gsl::span<int, 3>, const int(&)[3]>");
static_assert(
    !std::is_constructible<gsl::span<int, 3>, double*, std::size_t>::value,
...
static_assert(
    !std::is_constructible<gsl::span<int, 3>, double*, const int*>::value,
    "!std::is_constructible<gsl::span<int, 3>, double*, const int*>");
static_assert(
    !std::is_constructible<gsl::span<int, 3>, double*, double*>::value,
...
static_assert(
    !std::is_constructible<gsl::span<int, 3>, std::size_t, int*>::value,
    "!std::is_constructible<gsl::span<int, 3>, std::size_t, int*>");
static_assert(
    !std::is_constructible<gsl::span<int, 3>, std::size_t, std::size_t>::value,
    "!std::is_constructible<gsl::span<int, 3>, std::size_t, std::size_t>");
static_assert(
    !std::is_constructible<gsl::span<int, 3>, std::array<double, 3>&>::value,
    "!std::is_constructible<gsl::span<int, 3>, std::array<double, 3>&>");
static_assert(
    !std::is_constructible<gsl::span<int, 3>, std::array<int, 500>&>::value,
    "!std::is_constructible<gsl::span<int, 3>, std::array<int, 500>&>");
static_assert(
    !std::is_constructible<gsl::span<int, 3>, const std::array<int, 3>&>::value,
    "!std::is_constructible<gsl::span<int, 3>, const std::array<int, 3>&>");
static_assert(
    !std::is_constructible<gsl::span<int, 3>,
                           const gsl::span<int, 500>&>::value,
...
static_assert(!std::is_constructible<gsl::span<int, 3>,
                                     const gsl::span<const int, 3>&>::value,
              "!std::is_constructible<gsl::span<int, 3>, const gsl::span<const "
              "int, 3>&>");
static_assert(!std::is_constructible<gsl::span<int, 3>,
                                     const gsl::span<const int, 500>&>::value,
              "!std::is_constructible<gsl::span<int, 3>, const gsl::span<const "
...
static_assert(
    !std::is_constructible<gsl::span<int, 3>,
                           const gsl::span<double, 3>&>::value,
    "!std::is_constructible<gsl::span<int, 3>, const gsl::span<double, 3>&>");

static_assert(
    !std::is_constructible<gsl::span<const int>, double (&)[3]>::value,
    "!std::is_constructible<gsl::span<const int>, double(&)[3]>");
static_assert(
    !std::is_constructible<gsl::span<const int>, std::array<double, 3>&>::value,
    "!std::is_constructible<gsl::span<const int>, std::array<double, 3>&>");
static_assert(!std::is_constructible<gsl::span<const int>,
                                     const gsl::span<double, 3>&>::value,
              "!std::is_constructible<gsl::span<const int>, const "
              "gsl::span<double, 3>&>");

static_assert(!std::is_constructible<gsl::span<const int, 3>,
                                     const gsl::span<int, 500>&>::value,
              "!std::is_constructible<gsl::span<const int, 3>, const "
              "gsl::span<int, 500>&>");
static_assert(!std::is_constructible<gsl::span<const int, 3>,
                                     const gsl::span<const int, 500>&>::value,
              "!std::is_constructible<gsl::span<const int, 3>, const "
              "gsl::span<const int, 500>&>");
static_assert(!std::is_constructible<gsl::span<const int, 3>,
                                     const gsl::span<double, 3>&>::value,
              "!std::is_constructible<gsl::span<const int, 3>, const "
              "gsl::span<double, 3>&>");
...
static_assert(
    !std::is_constructible<gsl::span<Base>, std::array<Derived, 3>&>::value,
    "!std::is_constructible<gsl::span<Base>, std::array<Derived, 3>&>");
static_assert(
    !std::is_constructible<gsl::span<Base>, std::vector<Derived>&>::value,
    "!std::is_constructible<gsl::span<Base>, std::vector<Derived>&>");
static_assert(
    !std::is_constructible<gsl::span<Base>, const gsl::span<Derived>&>::value,
    "!std::is_constructible<gsl::span<Base>, const gsl::span<Derived>&>");
static_assert(
    !std::is_constructible<gsl::span<Base>,
                           const gsl::span<Derived, 3>&>::value,
    "!std::is_constructible<gsl::span<Base>, const gsl::span<Derived, 3>&>");

static_assert(
    !std::is_constructible<gsl::span<Base, 3>,
                           const gsl::span<Derived, 3>&>::value,
...
static_assert(
    !std::is_constructible<gsl::span<Base, 3>, std::array<Derived, 3>&>::value,
    "!std::is_constructible<gsl::span<Base, 3>, std::array<Derived, 3>&>");

static_assert(
    !std::is_constructible<gsl::span<const Base>, Derived (&)[3]>::value,
    "!std::is_constructible<gsl::span<const Base>, Derived(&)[3]>");
static_assert(
    !std::is_constructible<gsl::span<const Base>, const Derived (&)[3]>::value,
    "!std::is_constructible<gsl::span<const Base>, const Derived(&)[3]>");
static_assert(
    !std::is_constructible<gsl::span<const Base>,
                           std::array<Derived, 3>&>::value,
    "!std::is_constructible<gsl::span<const Base>, std::array<Derived, 3>&>");
static_assert(!std::is_constructible<gsl::span<const Base>,
                                     const std::array<Derived, 3>&>::value,
              "!std::is_constructible<gsl::span<const Base>, const "
              "std::array<Derived, 3>&>");
static_assert(
    !std::is_constructible<gsl::span<const Base>,
                           const gsl::span<Derived>&>::value,
...
static_assert(!std::is_constructible<gsl::span<const Base>,
              ...
              "gsl::span<Derived, 3>&>");
static_assert(!std::is_constructible<gsl::span<const Base>,
                                     const gsl::span<const Derived>&>::value,
              "!std::is_constructible<gsl::span<const Base>, const "
              "gsl::span<const Derived>&>");
static_assert(!std::is_constructible<gsl::span<const Base>,
                                     const gsl::span<const Derived, 3>&>::value,
              "!std::is_constructible<gsl::span<const Base>, const "
              "gsl::span<const Derived, 3>&>");

static_assert(!std::is_constructible<gsl::span<const Base, 3>,
                                     const gsl::span<Derived, 3>&>::value,
              "!std::is_constructible<gsl::span<const Base, 3>, const "
              "gsl::span<Derived, 3>&>");
static_assert(!std::is_constructible<gsl::span<const Base, 3>,
                                     const gsl::span<const Derived, 3>&>::value,
              "!std::is_constructible<gsl::span<const Base, 3>, const "
...
static_assert(
    !std::is_constructible<gsl::span<const Derived>,
                           std::array<Base, 3>&>::value,
    "!std::is_constructible<gsl::span<const Derived>, std::array<Base, 3>&>");
static_assert(!std::is_constructible<gsl::span<const Derived>,
                                     const std::array<Base, 3>&>::value,
              "!std::is_constructible<gsl::span<const Derived>, const "
              "std::array<Base, 3>&>");

// Explicit construction enabled in P1976R2
static_assert(
    std::is_constructible<gsl::span<int, 3>, const gsl::span<int>&>::value,
    "std::is_constructible<gsl::span<int, 3>, const gsl::span<int>&>");
static_assert(
    std::is_constructible<gsl::span<const int, 3>,
                          const gsl::span<int>&>::value,
...
static_assert(
    std::is_nothrow_copy_constructible<gsl::span<const int, 3>>::value,
...
static_assert(
    std::is_convertible<const int (&)[3], gsl::span<const int>>::value,
...
static_assert(
    std::is_convertible<const gsl::span<int>&, gsl::span<const int>>::value,
    "std::is_convertible<const gsl::span<int>&, gsl::span<const int>>");

static_assert(
    std::is_convertible<const gsl::span<int, 3>&, gsl::span<int>>::value,
    "std::is_convertible<const gsl::span<int, 3>&, gsl::span<int>>");
static_assert(
    std::is_convertible<const gsl::span<int, 3>&, gsl::span<int, 3>>::value,
    "std::is_convertible<const gsl::span<int, 3>&, gsl::span<int, 3>>");
static_assert(
    std::is_convertible<const gsl::span<int, 3>&, gsl::span<const int>>::value,
    "std::is_convertible<const gsl::span<int, 3>&, gsl::span<const int>>");
static_assert(
    std::is_convertible<const gsl::span<int, 3>&,
                        gsl::span<const int, 3>>::value,
    "std::is_convertible<const gsl::span<int, 3>&, gsl::span<const int, 3>>");
static_assert(
    std::is_convertible<const gsl::span<int, 500>&, gsl::span<int>>::value,
    "std::is_convertible<const gsl::span<int, 500>&, gsl::span<int>>");
static_assert(
    std::is_convertible<const gsl::span<int, 500>&,
                        gsl::span<const int>>::value,
    "std::is_convertible<const gsl::span<int, 500>&, gsl::span<const int>>");

static_assert(
    std::is_convertible<const gsl::span<const int>&,
                        gsl::span<const int>>::value,
    "std::is_convertible<const gsl::span<const int>&, gsl::span<const int>>");

static_assert(std::is_convertible<const gsl::span<const int, 3>&,
                                  gsl::span<const int>>::value,
              "std::is_convertible<const gsl::span<const int, 3>&, "
              "gsl::span<const int>>");
static_assert(std::is_convertible<const gsl::span<const int, 3>&,
                                  gsl::span<const int, 3>>::value,
              "std::is_convertible<const gsl::span<const int, 3>&, "
              "gsl::span<const int, 3>>");
static_assert(std::is_convertible<const gsl::span<const int, 500>&,
                                  gsl::span<const int>>::value,
              "std::is_convertible<const gsl::span<const int, 500>&, "
              "gsl::span<const int>>");

static_assert(std::is_convertible<std::array<int, 3>&, gsl::span<int>>::value,
              "std::is_convertible<std::array<int, 3>&, gsl::span<int>>");
static_assert(
    std::is_convertible<std::array<int, 3>&, gsl::span<int, 3>>::value,
    "std::is_convertible<std::array<int, 3>&, gsl::span<int, 3>>");
static_assert(
    std::is_convertible<std::array<int, 3>&, gsl::span<const int>>::value,
    "std::is_convertible<std::array<int, 3>&, gsl::span<const int>>");

static_assert(
    std::is_convertible<const std::array<int, 3>&, gsl::span<const int>>::value,
...
template <typename U>
static constexpr bool AsWritableBytesCompilesFor<
    U, void_t<decltype(as_writable_bytes(declval<U>()))>> = true;
