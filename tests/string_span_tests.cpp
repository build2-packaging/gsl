...
TEST_CASE("string_span_tests, TestLiteralConstruction") {
...
TEST_CASE("string_span_tests, TestConstructFromStdString") {
  std::string s = "Hello there world";
  cstring_span<> v = s;
  CHECK(v.length() == static_cast<cstring_span<>::size_type>(s.length()));
}

TEST_CASE("string_span_tests, TestConstructFromStdVector") {
  std::vector<char> vec(5, 'h');
  string_span<> v{vec};
  CHECK(v.length() == static_cast<string_span<>::size_type>(vec.size()));
}

TEST_CASE("string_span_tests, TestStackArrayConstruction") {
...
TEST_CASE("string_span_tests, TestConstructFromConstCharPointer") {
  const char* s = "Hello";
  cstring_span<> v = ensure_z(s);
  CHECK(v.length() == 5);
}

TEST_CASE("string_span_tests, TestConversionToConst") {
...
TEST_CASE("string_span_tests, TestConversionFromConst") {
  char stack_string[] = "Hello";
...
TEST_CASE("string_span_tests, TestToString") {
  auto s = gsl::to_string(cstring_span<>{});
  CHECK(s.length() == static_cast<size_t>(0));

  char stack_string[] = "Hello";
  cstring_span<> v = ensure_z(stack_string);
  auto s2 = gsl::to_string(v);
  CHECK(static_cast<cstring_span<>::size_type>(s2.length()) == v.length());
  CHECK(s2.length() == static_cast<size_t>(5));
}

TEST_CASE("string_span_tests, TestToBasicString") {
  auto s = gsl::to_basic_string<char, std::char_traits<char>,
                                ::std::allocator<char>>(cstring_span<>{});
  CHECK(s.length() == static_cast<size_t>(0));

  char stack_string[] = "Hello";
  cstring_span<> v = ensure_z(stack_string);
  auto s2 = gsl::to_basic_string<char, std::char_traits<char>,
                                 ::std::allocator<char>>(v);
  CHECK(static_cast<cstring_span<>::size_type>(s2.length()) == v.length());
  CHECK(s2.length() == static_cast<size_t>(5));
}

TEST_CASE("string_span_tests, EqualityAndImplicitConstructors") {
  {
...
TEST_CASE("string_span_tests, ComparisonAndImplicitConstructors") {
  {
    cstring_span<> span = "Hello";
...
TEST_CASE("string_span_tests, ConstrutorsEnsureZ") {
...
TEST_CASE("string_span_tests, Constructors") {
...
TEST_CASE("string_span_tests, MoveConstructors") {
  // move string_span
  {
    cstring_span<> span = "Hello";
...
TEST_CASE("string_span_tests, Conversion") {
#ifdef CONFIRM_COMPILATION_ERRORS
  cstring_span<> span = "Hello";
...
TEST_CASE("string_span_tests, zstring") {
  std::set_terminate([] {
  ...
  // usage scenario: create zero-terminated temp file name and pass to a legacy
  // API
  {
    char buf[10];
...
TEST_CASE("string_span_tests, wzstring") {
  std::set_terminate([] {
  ...
  // create zspan from non-zero terminated string
  {
    ...
    const auto workaround_macro = [&]() {
      const wzstring_span<> zspan({buf, 1});
    };
    // EXPECT_DEATH(workaround_macro(), deathstring);
  }
...
TEST_CASE("string_span_tests, u16zstring") {
  std::set_terminate([] {
  ...
  // create zspan from non-zero terminated string
  {
    ...
    const auto workaround_macro = [&]() {
      const u16zstring_span<> zspan({buf, 1});
    };
    // EXPECT_DEATH(workaround_macro(), deathstring);
  }
...
TEST_CASE("string_span_tests, u32zstring") {
  std::set_terminate([] {
  ...
  // create zspan from non-zero terminated string
  {
    ...
    const auto workaround_macro = [&]() {
      const u32zstring_span<> zspan({buf, 1});
    };
    // EXPECT_DEATH(workaround_macro(), deathstring);
  }
...
TEST_CASE("string_span_tests, Issue305") {
  std::map<gsl::cstring_span<>, int> foo = {{"foo", 0}, {"bar", 1}};
  CHECK(foo["foo"] == 0);
  CHECK(foo["bar"] == 1);
}

TEST_CASE("string_span_tests, char16_t_type") {
...
TEST_CASE("string_span_tests, char32_t_type") {
...
TEST_CASE("string_span_tests, as_bytes") {
...
TEST_CASE("string_span_tests, as_writable_bytes") {
