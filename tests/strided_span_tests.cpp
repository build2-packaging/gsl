...
#else
  auto bounds =
      strided_bounds<1>(multi_span_index<1>{length}, multi_span_index<1>{2});
#endif
...
void iterate_second_slice(
    multi_span<int, dynamic_range, dynamic_range, dynamic_range> av) {
  ...
  for (auto i = 0; i < section.extent<0>(); ++i) {
    for (auto j = 0; j < section.extent<1>(); ++j)
      for (auto k = 0; k < section.extent<2>(); ++k) {
        auto idx =
            multi_span_index<3>{i, j, k};  // avoid braces in the CHECK macro
...
TEST_CASE("strided_span_tests, span_section_test") {
  int a[30][4][5];
...
TEST_CASE("strided_span_tests, span_section") {
  std::vector<int> data(5 * 10);
  std::iota(begin(data), end(data), 0);
  const multi_span<int, 5, 10> av =
  ...
  const strided_span<int, 2> av_section_2 =
      av_section_1.section({1, 2}, {2, 2});
...
TEST_CASE("strided_span_tests, strided_span_constructors") {
...
#else
      strided_span<const volatile int, 1> sav_cv{
          multi_span<const volatile int>{src}, strided_bounds<1>{2, 1}};
#endif
  ...
  // CHECK const-casting constructor
  {
    ...
    static_assert(std::is_convertible<const multi_span<int, 2>,
                                      multi_span<const int, 2>>::value,
    ...
    static_assert(std::is_convertible<const strided_span<int, 1>,
                                      strided_span<const int, 1>>::value,
                  "ctor is not implicit!");
  }
...
TEST_CASE("strided_span_tests, strided_span_slice") {
  std::vector<int> data(5 * 10);
  std::iota(begin(data), end(data), 0);
...
TEST_CASE("strided_span_tests, strided_span_column_major") {
...
TEST_CASE("strided_span_tests, strided_span_bounds") {
  int arr[] = {0, 1, 2, 3};
...
#ifdef CONFIRM_COMPILATION_ERRORS
  {
    ...
    strided_span<int, 2> sav12{av.as_multi_span(dim<2>(), dim<2>()),
                               {{1}, {1}}};
    strided_span<int, 2> sav13{av.as_multi_span(dim<2>(), dim<2>()),
                               {{1}, {1, 1, 1}}};
    strided_span<int, 2> sav14{av.as_multi_span(dim<2>(), dim<2>()),
                               {{1, 1, 1}, {1}}};
  }
#endif
}

TEST_CASE("strided_span_tests, strided_span_type_conversion") {
  int arr[] = {0, 1, 2, 3};
  multi_span<int> av(arr);
  ...
  // retype strided array with not enough elements - last dimension of the array
  // is too small
  {
  ...
  // retype strided array with not enough elements - last dimension does not
  // divide by the new typesize
  {
  ...
  // retype strided array with not enough elements - strides does not divide by
  // the new typesize
  {
...
TEST_CASE("strided_span_tests, empty_strided_spans") {
  std::set_terminate([] {
...
TEST_CASE("strided_span_tests, strided_span_section_iteration") {
...
TEST_CASE("strided_span_tests, dynamic_strided_span_section_iteration") {
  auto arr = new int[8];
  for (int i = 0; i < 4; ++i) {
    arr[2 * i] = 4 + i;
    arr[2 * i + 1] = i;
...
TEST_CASE("strided_span_tests, strided_span_section_iteration_3d") {
  int arr[3][4][2]{};
  for (auto i = 0; i < 3; ++i) {
    for (auto j = 0; j < 4; ++j)
...
TEST_CASE("strided_span_tests, dynamic_strided_span_section_iteration_3d") {
  ...
  {
    auto av =
        as_multi_span(as_multi_span(arr, 24), dim<3>(), dim<4>(), dim<2>());
    iterate_second_slice(av);
  }
...
TEST_CASE("strided_span_tests, strided_span_conversion") {
  std::set_terminate([] {
  ...
  auto av =
      as_multi_span(as_bytes(as_multi_span(&arr[0], 4)), dim(d1), dim(d2));

  CHECK(av.bounds().index_bounds()[0] == 4);
  CHECK(av.bounds().index_bounds()[1] == 12);

  // get the last 4 columns
  auto section = av.section(
      {0, 2 * s}, {4, s});  // { { arr[0].c[0], arr[0].c[1], arr[0].c[2],
                            // arr[0].c[3] } , { arr[1].c[0], ... } , ...
                            // }

  // convert to array 4x1 array of integers
  auto cs =
      section.as_strided_span<int>();  // { { arr[0].c }, {arr[1].c } , ... }
  ...
  strided_span<int, 2> transposed{cs.data(), cs.bounds().total_size(),
                                  reverse_bounds};
